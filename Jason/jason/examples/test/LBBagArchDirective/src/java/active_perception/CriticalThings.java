...
public class CriticalThings{ 
    ...
    //Get plans that has any belief marked with ap in context; changed LBB
    public static PlansApBel getPlansApBel(Agent outerContent){
        PlansApBel plans_ap_bel = new PlansApBel(); //1. create class PlansCT
        for(Plan p : outerContent.getPL()){
            //FIX: Substitute bellow for a code that search a certain annotation
    ...
    //FIX: LBB first (failed) test
    public static void parseCPs(Agent outerContent, Set<Trigger> trigger_set, Atom annot){
        Pred cp_label = new Pred(createLiteral("cp"));
        int i=0;
        for(Plan p: outerContent.getPL().getPlans()){
            if(p.getLabel() == cp_label){   //If it is critical plan
                //1. remove it from the Plan Library
                outerContent.getPL().getPlans().remove(i); //FIX: find improved version, this removes from the list but not all entries
                //2. add it to the CLM
                Term label = p.getLabel().clone();
    ...
    //LBB second test - works fine locating the plan; now must delete it
    ...
    //Adding new plans +!update(X[ap(T)]): not critical_things.isUpdated(X[ap(T)]) <- ?X[ap].
    //and +!update(X).
    public static void addUpdatePlan(Agent outerContent){
        ...
        InternalActionLiteral isUpdated = new InternalActionLiteral("critical_things.isUpdated");
        ...
        CriticalThings.addNewPlan(outerContent, update_trigger, not_isUpdated, update_body);
        CriticalThings.addNewPlan(outerContent, update_trigger, null);
    }
