...
public class DemoEmbeddedAgentArch extends DefaultEmbeddedAgArch {
    ...
    static {
        Map.of(
            5, List.of("cp3,Critical","cp1,Marginal"),
            4, List.of("cp0,Critical", "cp3,Severe"),
            3, List.of("cp0,Severe", "cp3,Marginal"),
            2, List.of("cp0,Marginal"),
            1, List.<String>of() 
    ...
    private int getPriority(String cp, String severity) {
        return cpToPriority.getOrDefault(cp + "," + severity, switch (severity) {
            case "Marginal" -> 1;
            case "Severe"   -> 2;
            case "Critical" -> 3;
            default         -> 0;
