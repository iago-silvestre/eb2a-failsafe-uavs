...
public class DemoEmbeddedAgentArch extends DefaultEmbeddedAgArch {
    ...
    public Boolean[] perceiveCP() {
        ...
        long tEvent = eventTimeNs.getAndSet(-1);
        if (tEvent != -1) {
            bb.add(ASSyntax.createLiteral("cp0"));

            long detectionDelay = System.nanoTime() - tEvent;
            System.out.println("Event detection delay (ns): " + detectionDelay);
    ...
    private void scheduleNextAsyncEvent() {
        ...
        asyncEventExecutor.schedule(() -> {
            // event happens RIGHT HERE (arbitrary real time)
