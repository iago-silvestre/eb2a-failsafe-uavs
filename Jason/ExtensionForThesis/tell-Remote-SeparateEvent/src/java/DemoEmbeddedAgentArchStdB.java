...
public class DemoEmbeddedAgentArchStdB extends DefaultEmbeddedAgArch {
    ...
    private AsyncEventGenerator eventGenerator;
    private Thread eventThread;
    private boolean generatorStarted = false;

    
    public DemoEmbeddedAgentArchStdB() {
        super();
    ...
    public synchronized void injectCp0Event() {
        ReactionTime.markEvent();
        BeliefBase bb = getTS().getAg().getBB();
        Trigger goal = new Trigger(TEOperator.add, TEType.achieve, ASSyntax.createLiteral("tell"));
        getTS().getC().addEvent(new Event(goal, null));
        //Literal cpLiteral = ASSyntax.createLiteral("cp0");
        //bb.add(cpLiteral);
    }

    private void ensureGeneratorStarted() {
        if (!generatorStarted) {
            eventGenerator = new AsyncEventGenerator(this);
            eventThread = new Thread(eventGenerator, "AsyncEventGenerator");
    ...
    public Collection<Literal>perceive() {
        ...
        if (this.cpIterationCounter == null) {
            this.cpIterationCounter = 0;
            this.nextCPTrigger = 10 + new java.util.Random().nextInt(100);
            this.cpCount = 0;
        }

        this.cpIterationCounter++;
        /*if (this.cpIterationCounter >= this.nextCPTrigger ) {
            ReactionTime.markEvent();
            //String perceptName = "tell";  // simplified belief
