...
public class MyRosMaster extends RosMaster {
    ...
    private static final int MAX_REACTIONS = 100;

    private static final Vector<String> reactionData = new Vector<>();
    private static int reactionCount = 0;

    private static final String LOG_FILE_PATH = "reaction_times.csv";
    ...
    private static void logReactionsToFile() {
        try (FileWriter writer = new FileWriter(LOG_FILE_PATH, false)) {

            writer.write(
                "tEvent,tPerception,tReaction," +
                "eventToPerception,eventToReaction,perceptionToReaction\n"
            );

            for (String r : reactionData) {
                writer.write(r + "\n");
            }

            writer.flush(); // optional but explicit
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
	...
	public boolean execEmbeddedAction(String actionName, Object[] args, Unifier un) {		
        ...
        if (actionName.equals("cp0_Minor")) {
            ...
            long tReaction = System.nanoTime();
            long tEvent    = ReactionTime.getEventTime();
            long tPerc     = ReactionTime.getPerceptionTime();

            long eventToPerception    = tPerc     - tEvent;
            long eventToReaction      = tReaction - tEvent;
            long perceptionToReaction = tReaction - tPerc;
            ...
            reactionData.add(
                ...
                eventToPerception + "," +
                eventToReaction + "," +
                perceptionToReaction
            );
            
            reactionCount++;
            //String response = Integer.toString(reactionCount);
            //if(reactionCount<5){
                ((DefaultRos4EmbeddedMas) microcontroller).rosWrite("/agent_detected_failure_uav1","std_msgs/String","1");
            //}
            System.out.println(reactionCount);

            // EXACTLY when we hit 100
            if (reactionCount == MAX_REACTIONS) {

                logReactionsToFile();   // flush results
                ReactionTime.clearEvent();

                System.out.println("Collected " + MAX_REACTIONS + " reactions. Exiting JVM.");

                System.exit(0);         // ⬅ clean JVM termination
            }
        ...
        /*if (actionName.equals("cp0_Minor")) {
            ...
            //System.out.println("cp0_Minor started at " + getCurrentTime());
        ...
        if (actionName.equals("cp1_Major")) {
            ...
            //System.out.println("cp1_Major started at " + getCurrentTime());
			String response = Integer.toString(lastResponseIndex);
            lastResponseIndex++;  // increment for each reaction
            System.out.println(response);
			((DefaultRos4EmbeddedMas) microcontroller).rosWrite("/agent_detected_failure_uav1","std_msgs/String",response);
        ...
        if (actionName.equals("cp2_Catastrophic")) {
            System.out.println("end: " + getCurrentTime());
            //System.out.println("cp2_Catastrophic started at " + getCurrentTime());
			String response = Integer.toString(lastResponseIndex);
            lastResponseIndex++;  // increment for each reaction
            System.out.println(response);
			((DefaultRos4EmbeddedMas) microcontroller).rosWrite("/agent_detected_failure_uav1","std_msgs/String",response);
        ...
        if (actionName.equals("testeC1")) {
            try {
                for (int i = 1; i <= 10; i++) {   // 10 × 500ms = 5000ms = 5s
                    ...
                    System.out.println("Timer: " + (i * 500) + " ms");
                }
            } catch (InterruptedException e) {
                e.printStackTrace();
                Thread.currentThread().interrupt(); // restore interrupt status
            }
			System.out.println("testeC1");
            return true;
		}

        if (actionName.equals("testeC1nd")) {
			System.out.println("testeC1");
            return true;
		}
        if (actionName.equals("testeC2")) {
            try {
                for (int i = 1; i <= 10; i++) {   // 10 × 500ms = 5000ms = 5s
                    Thread.sleep(500);            // wait 500ms
                    System.out.println("Timer: " + (i * 500) + " ms");
            ...
            System.out.println("testeC2");
            return true;
        }

        if (actionName.equals("testeC2nd")) {
			System.out.println("testeC2");
            return true;
		}

        if (actionName.equals("testeC3")) {
            try {
                for (int i = 1; i <= 10; i++) {   // 10 × 500ms = 5000ms = 5s
                    Thread.sleep(500);            // wait 500ms
                    System.out.println("Timer: " + (i * 500) + " ms");
			...
			System.out.println("testeC3");
            return true;
		}
        if (actionName.equals("testeC3nd")) {
			System.out.println("testeC3");
            return true;
		}
